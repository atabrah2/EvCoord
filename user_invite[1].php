...
if ($_SESSION['valid'] != true) {
    die("failure: you must be logged in to invite a user.");
}
?>
<!DOCTYPE html>
<html lang="en">
...
//security checks
if ($status > 1) {
    die("you cannot invite users for an expired event.");
}
if ($_SESSION['id'] != $team_leader_id) {
    die("you do not have permission to invite users onto this team.");
}
...
if (($event_max_skill > 0 && $user_min_skill > $event_max_skill) || $status === 2 || $_SESSION['id'] == $event_organizer_id) {
...
$sql="SELECT * FROM Invite WHERE user_id = '$user_id' AND team_name = '$team_name' AND event_name = '$event_name' AND event_start = '$event_start' AND event_end = '$event_end'";
$res=mysql_query($sql);
if (mysql_num_rows($res) > 0) {
    print("There is already a pending invite. Please cancel it before inviting this user.");
    print("<br/>");
    print("<button class=\"cancelbutton\" onclick=\"window.location.href='user_page.php?user_id=$user_id&team_name=$team_name&event_name=$event_name&event_start=$event_start&event_end=$event_end'\">Back</button>");
    die();
}
?>
...
<div class="container">
    <?php
    print("<h1>Invitation for user $user_id</h1>");
    print("<p>To join team: $team_name</p>");
    print("<form action=\"user_invite_submit.php?team_name=$team_name&event_name=$event_name&event_start=$event_start&event_end=$event_end&user_id=$user_id\" method=\"post\" target=\"_top\">");
    ?>
        <label style=color:black;font-family:verdana><b>Role</b></label>
        <select name="roleName">
            <?php
            //query available roles
            if ($event_max_skill > 0) {
            ...
            if (mysql_num_rows($res) == 0) {
                die("Catastrophic failure: You cannot invite this man.");
            }
        ...
        <button "class="button10" type="submit">Send Invite</button>
    </form>
</div>
