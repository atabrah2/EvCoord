...
<head>
    ...
    <title>Team Update Submit</title>
...
//query for our membership (and as a side effect, team)
...
if (mysql_num_rows($res) == 0) {
    die("No such team ($team_name, $event_name, $event_start, $event_end) exists or has not given you permission to post.");
}
$user_id = "{$_SESSION['id']}";
$sql = "INSERT INTO Team_Update(team_name, event_name, event_start, event_end, posted_time, poster_id, description)
        VALUES ('$team_name', '$event_name', '$event_start', '$event_end', NOW(), '$user_id', '$post_description')";
$res = mysql_query($sql);
if (!$res) {
