...
$sql="SELECT *
      FROM Team
      WHERE name = '$team_name' AND event_name = '$event_name' AND event_start = '$event_start' AND event_end = '$event_end' AND leader_id = '$user_id'";
$res=mysql_query($sql);
if (!$res) {
    die("leader validation failed.");
}
if (mysql_num_rows($res) > 0) {
    print("<p>You cannot leave your team as a leader.</p>");
...
$res = mysql_query($sql);
if (!$res) {
    print ("Attempt to leave failed");
}
else {
    print("You have successfully left $team_name");
}
mysql_close();
print("<button class=\"cancelbutton\" onclick=\"window.location.href='team_members.php?team_name=$team_name&event_name=$event_name&event_start=$event_start&event_end=$event_end'\">Back</button>");
?>
